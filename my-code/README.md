...
Now you can rename `src/App.less` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/App.less`. Since `src/App.jsx` still imports `src/App.less`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/App.less` will be regenerated.
